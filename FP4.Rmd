...
```{r}
uplift <- read_csv("11_8_data_clean.csv")
...
efcrank$efc_index <- 1:nrow(efcrank)
salaryrank <- efcrank[order(efcrank$job_salary,decreasing=TRUE),]
salaryrank$salary_index <- 1:nrow(salaryrank)
salaryrank$change <- salaryrank$efc_index-salaryrank$salary_index
```
